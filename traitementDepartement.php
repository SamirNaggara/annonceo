...
$tab['afficher'] = '';
$tab['pourLeChampVille'] = '';
$tab['faireApparaitreVille'] = '';


if (isset($_POST['departement'])){
    if ($_POST['departement'] != 'toutes'){
        $tab['faireApparaitreVille'] .= 'ok';
    }
    $tab['afficher'] .= $_POST['departement'];
    $optionDeMaBoucle = "";
    
        foreach(villes($_POST['departement'], $lesVilles) as $laVille){
            $optionDeMaBoucle .=  '<option>' . $laVille . '</option>';
    }
        
    $tab['pourLeChampVille'] .= '<option value="toutes">Toutes les villes</option>' . $optionDeMaBoucle;          
}
